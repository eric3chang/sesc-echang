...
struct mint_barrier_str
{
	HASH_SET<uint32_t> waitingSet;
};

extern HASH_MAP<uint32_t,mint_barrier_str> mint_barrier_set;
...
#define IS_DUMMY_INST	0x8000  /* marks an instruction as not generating a dynamic instruction */

/* bit mask for branches and jumps */
#define BRANCH_OR_JUMP  (IS_BRANCH | IS_JUMP | IS_JUMPREG)

extern uint32_t processorSkewCounter[256];
...
extern PFPI simOpTable[];

PFPI simOpHandler(icode_ptr);
