...
#if (defined DARWIN) || (defined CYGWIN) || (defined _MSC_VER)
...
#define MAX_BARRIER_DELAY 100

HASH_MAP<uint32_t,mint_barrier_str> mint_barrier_set;
...
#ifdef _MSC_VER
void conv_stat32_native2glibc(const struct _stat *statp, struct glibc_stat32 *stat32p)
#else
...
#endif
{
  ...
  stat32p->st_blksize = SWAP_WORD(4096);
  stat32p->st_blocks  = SWAP_WORD(8);
}
#ifdef _MSC_VER
void conv_stat64_native2glibc(const struct _stat *statp, struct glibc_stat64 *stat64p)
#else
void conv_stat64_native2glibc(const struct stat *statp, struct glibc_stat64 *stat64p)
#endif
{
  stat64p->st_dev   = SWAP_WORD(statp->st_dev);
  stat64p->st_ino   = SWAP_LONG((uint64_t)statp->st_ino); 
...
func_desc_t Func_subst[] = {
  ...
  {"sesc_barrier",            mint_sesc_barrier,               1, OpExposed},
...
OP(mint_assert_fail)
{
  ...
  std::cout << " on thread " << pthread->getPid() << std::endl;
  I(0);
...
OP(mint_sesc_spawn){
  ...
  if (Maxpid < cpid)
    Maxpid = cpid;
// #ifdef DEBUG_VERBOSE
  printf("sesc_spawn( 0x%lx, 0x%lx, 0x%lx ) pid = %d, child pid = %d, child context = %lx\n",
         (unsigned long)entry,(unsigned long)arg,(unsigned long)flags,ppid,cpid,(unsigned long)child);
//#endif 
...
{
#ifdef _MSC_VER
	fatal("Operation not yet supported in transitioned version at line : %d",__LINE__);
#else
#if (defined __LP64__)
...
{
#ifdef _MSC_VER
	printf("Improvixing a call to fcntl, returning std out/in\n");
	...
	case(1):
		if(!(fd >= 0 && fd <= 2))
			fatal("call was for a FD that is not an std pipe\n");
		pthread->setRetVal(fd);
		return pthread->getRetIcode();
	default:
		fatal("Operation not yet supported in transitioned version at line : %d",__LINE__);
	}
...
#ifdef _MSC_VER
  err = _open((const char*)(pthread->virt2real(pathname)), flags, mode);
#else
...
#ifdef _MSC_VER
  err=_write(fd,(const void *)(pthread->virt2real(buf)),count);
#else
...
OP(mint_fxstat64)
{
  ...
  if(addr==0)
    fatal("fstat called with a null pointer (ip=0x%x)\n", picode->addr);
  struct _stat stat_native;
  retVal=_fstat(fd,&stat_native);
  pthread->setRetVal(retVal);
  if(retVal==-1)
    pthread->setErrno(errno);
  struct glibc_stat64 stat64p;     
...
OP(mint_times) {
  ...
  sysCall->exec(pthread,picode);
    ...
    {
      char *cad1 = (char *)malloc(sizeof(struct tms));
...
OP(mint_malloc)
{
  ...
  sysCall->exec(pthread,picode);
#else // Begin (defined TLS) else block
  VAddr addr=TMInterface::DoAllocate(pthread->getPid(),size);
//VAddr addr=pthread->getHeapManager()->allocate(size);
  // Set errno on error to be POSIX compliant
	...
	{
		bool overlap = false;
		for(VAddr i = addr; i < addr + size; i++)
		{
			if(heapMask[i - Heap_start])
			{
				overlap = true;
				break;
			}
		...
		if(overlap)
		{
			...
			printf("Proc,Thread:%d-%d",gbCurrentProcessor,gbCurrentPid);
#endif
			printf("Overlap discovered for allocation returning address %d of size %d\n",addr,size);
		}
		for(VAddr i = addr; i < addr + size; i++)
		{
			heapMask[i - Heap_start] = true;
		}
		allocSizeMap[addr] = size;
	}
	else
	{
		printf("No memory for size %d\n",size);
	}
#endif
  if(!addr)
...
//VAddr addr=pthread->getHeapManager()->allocate(totalSize);
  // Set errno on error to be POSIX compliant
	...
	{
		bool overlap = false;
		for(VAddr i = addr; i < addr + totalSize; i++)
		{
			if(heapMask[i - Heap_start])
			{
				overlap = true;
...
OP(mint_free)
{
  ...
  if(!addr)
  {
	  printf("Attempted to free NULL");
  }
  else if(allocSizeMap.find(addr) == allocSizeMap.end())
  {
	  printf("Failed to free.  Address not found for addr %d, was not allocated",addr);
  }
  else
  {
		int size = allocSizeMap[addr];
		if(addr)
		{
			bool missing = false;
			for(VAddr i = addr; i < addr + size; i++)
			{
				if(heapMask[i - Heap_start] == false)
				{
					missing = true;
					break;
				}
			...
			for(VAddr i = addr; i < addr + size; i++)
			{
				heapMask[i - Heap_start] = false;
			}
			allocSizeMap.erase(addr);
		}
	}
#endif
	if(addr){
  //pthread->getHeapManager()->deallocate(addr);
	TMInterface::DoFree(pthread->getPid(),addr);
  }
  // There should be no context switch
  I(pthread->getPid()==thePid);
  // Return from the call
...
OP(mint_sesc_barrier)
{
	VAddr barrierAddr = (VAddr)pthread->getIntArg1();
	uint32_t procCount = (uint32_t)pthread->getIntArg2();
	Pid_t pid = pthread->getPid();
	if(TMInterface::IsTransacting(pid))
	{
		std::cout << "Waiting for proc " << pid << " to finish a transaction" << std::endl;
		osSim->cpus.getProcessor((CPU_t)pid)->BusyWaitFor(100);
		return picode;
	}
	mint_barrier_set[barrierAddr].waitingSet.insert(pid);
	if(mint_barrier_set[barrierAddr].waitingSet.size() >= procCount)
	{//MARK fix this to find procs instead of pids for future use
		I(mint_barrier_set[barrierAddr].waitingSet.size() == procCount);
		std::cout << "Processor " << pid << " has triggered a release" << std::endl;
		...
		for(HASH_SET<uint32_t>::iterator it = mint_barrier_set[barrierAddr].waitingSet.begin(); it != mint_barrier_set[barrierAddr].waitingSet.end(); it++)
		{
			if(*it != pid)
			{
				...
				{
					osSim->cpus.getProcessor((CPU_t)*it)->StopBusyWait();
				}
				else
				{
					osSim->cpus.getProcessor((CPU_t)*it)->StopBusyWaitIn((TimeDelta_t)(rand() % MAX_BARRIER_DELAY));
				}
			}
			std::cout << "Releasing processor " << *it << " from barrier" << std::endl;
		}
		mint_barrier_set[barrierAddr].waitingSet.clear();
	}
	else
	{
		std::cout << "Stalling processor " << pid << " at barrier" << std::endl;
		osSim->cpus.getProcessor(pid)->StartBusyWait();
	}
	return pthread->getRetIcode();
}
