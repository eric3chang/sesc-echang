...
#define INSTR_CHECKPOINT_INCREMENT 500000
...
void FetchEngine::realFetch(IBucket *bucket, int32_t fetchMax)
{
  ...
  }while(n2Fetched>0 && flow.currentPid()==myPid && !(osSim->cpus.getProcessor(flow.currentPid())->IsBusyWaiting()));

  ushort tmp = FetchWidth - n2Fetched;

  totalnInst+=tmp;
  if(totalnInst > instrCheckpoint)
  {
	  std::cout << "Simulated " << totalnInst << " instructions so far at clock " << globalClock << std::endl;
	  instrCheckpoint += INSTR_CHECKPOINT_INCREMENT;
  }
