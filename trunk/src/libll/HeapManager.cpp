...
VAddr HeapManager::allocate(size_t size)
{
  ...
  {
	  std::cout << "No free memory for heap allocation" << std::endl;
    return 0;
  }
...
size_t HeapManager::getAllocationSize(VAddr addr)
{
  BlocksByAddr::iterator busyIt=busyByAddr.find(BlockInfo(addr,0));
  I(busyIt!=busyByAddr.end());
  return busyIt->size;
}
void HeapManager::ForceBlockPopulation(VAddr start, size_t size)
{
	allocate(start,size);
}
HeapManager::BlocksByAddr::iterator HeapManager::GetAllocationIterator()
{
	return busyByAddr.begin();
}
unsigned int HeapManager::GetAllocationCount()
{
	return busyByAddr.size();
}
HeapManager::BlocksByAddr::iterator HeapManager::GetAllocationDetails(HeapManager::BlocksByAddr::iterator allocIt, VAddr& address, size_t& size)
{
	BlocksByAddr::iterator ret = allocIt;
	ret++;
	address = allocIt->addr;
	size = allocIt->size;
	return ret;
}
