...
uint32_t ThreadContext::globalBusyWaitStamp = 1;
...
void ThreadContext::copy(const ThreadContext *src)
{
  ...
  isBusyWaiting = src->isBusyWaiting;
  busyWaitStamp = src->busyWaitStamp;
}
...
void ThreadContext::free(void)
{
  ...
  // zero in the child process).
   std::cout << "ThreadContext::free: "
         << this << " " << mainThreadContext << " " << nThreads << std::endl;
...
int64_t  MintFuncArgs::getInt64(void){
  ...
  if(curPos<16){
    ...
    retVal|=myContext->getIntReg((IntRegName)(4+curPos/4+1))&0xFFFFFFFFll;
  }else{
    RAddr addr=myContext->virt2real(myContext->getStkPtr())+curPos;
...
void ThreadContext::dumpCallStack(void){
  ...
  for(size_t i=0;i<callStack.size();i++)
    printf("  Entry 0x%08llx from 0x%08llx with sp 0x%08llx tail %d Name %s File %s\n",
	   (long long)(callStack[i].entry),(long long)(callStack[i].ra),
	   (long long)(callStack[i].sp),callStack[i].tailr,
