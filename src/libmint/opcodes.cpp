 ...
 */
uint32_t processorSkewCounter[256] = {0};
...
#endif
	{ "tm_simOp_op", tm_simOp_op, {REG0, REG0 | MOD0, 0, 0}, IS_DUMMY_INST, 0, 1 },
	{ "tm_transOp_op", tm_transOp_op, {REG0, REG0 | MOD0, 0, 0}, IS_DUMMY_INST, 0, 1},
	{ "tm_specOp_op", tm_specOp_op, {REG0, REG0 | MOD0, 0, 0}, IS_DUMMY_INST, 0, 1 }
};
...
PFPI simOpTable[] = 
{
	SimOp_BeginRabbit,
	SimOp_EndRabbit,
	SimOp_TakeCheckpoint,
	SimOp_Pause,
	SimOp_Skew,
};

PFPI simOpHandler(icode_ptr picode)
{
	if(picode->immed < 5)
	{
		return simOpTable[picode->immed];
	}
	else
	{
		return DoNothingOp;
...
OP(SimOp_BeginRabbit)
{
	osSim->RabbitModeStart();
	return picode->next;
}
OP(SimOp_EndRabbit)
{
	osSim->RabbitModeStop();
	return picode->next;
}
OP(SimOp_TakeCheckpoint)
{
	std::cout << "checkpointing..." << std::endl;
	osSim->SaveCheckpoint();
	return picode->next;
}
OP(SimOp_Pause)
{
	std::cout << "Pausing at SimOp pause point, press a key to continue" << std::endl;
	std::cin.clear();
	std::cin.get();
	return picode->next;
}
OP(SimOp_Skew)
{
	if(processorSkewCounter[pthread->getPid()])
	{
		pthread->BusyWaitFor(processorSkewCounter[pthread->getPid()]);
		processorSkewCounter[pthread->getPid()] = 0;
	}
	return picode->next;
}
