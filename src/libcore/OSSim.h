...
extern int gbCurrentPid;
extern int gbCurrentProcessor;
#endif
#ifdef DEBUG_HEAP_DIAG
extern bool* heapMask;
extern std::map<VAddr,int> allocSizeMap;
#endif

extern uint64_t globalInstructionStampCounter;

class GProcessor;

class OSSim {
private:
	int rabbitModeCounter;
...
public:
  ...
  void eventSimulationMark(int32_t id,Pid_t pid) {
    if(idSimMarks.find(id)==idSimMarks.end()) {
      ...
      idSimMarks[id].end = (unsigned int)((~0UL)-1);
    }
  ...
  void SaveCheckpoint();
  bool LoadCheckpoint();
  void RabbitModeStart();
  void RabbitModeStop();

  bool isGoingRabbit(){return rabbitModeCounter > 0;}

	Memory::MemorySystem MemSys_system;
