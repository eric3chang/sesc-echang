...
void GStatsHist::reportValue() const
{
  ...
  long double div = (long double)cumulative; // cummulative has 64bits (double has 54bits mantisa)
  div /= numSample;
...
void GStatsHist::sample(uint32_t key, long long weight)
{
  if(H.find(key)==H.end())
    H[key]=0;
...
{
  GStatsHist::sample(lastKey, (long long) (globalClock-lastUpdate));
  lastUpdate = globalClock;
  lastKey = key;
}
...
void GStatsEventTimingHist::begin_sample(uint64_t id)
{
...
void GStatsEventTimingHist::commit_sample(uint64_t id)
{
...
void GStatsEventTimingHist::remove_sample(uint64_t id)
{
  EventHistory::iterator it_begin;
  Time_t startTime;
