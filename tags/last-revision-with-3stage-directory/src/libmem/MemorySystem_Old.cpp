...
#include "../MemSys/Devices/SESCProcessorInterface.h"
...
{ 
  ...
  if (!strcasecmp(device_type, k_cache) || !strcasecmp(device_type,k_icache)) {
    ...
    return NULL; 
  } else if (!strcasecmp(device_type, "SESCProcessorInterface"))  {
	  Memory::SESCProcessorInterface* pi = new Memory::SESCProcessorInterface(device_descr_section,device_name);
	  new_memory_device = pi;
	  osSim->MemSys_system.SubmitInterfaceDevice(std::string(device_descr_section),pi);
  } else {
