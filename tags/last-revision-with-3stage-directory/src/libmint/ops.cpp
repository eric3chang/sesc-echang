 ...
 * the Free Software Foundation, 675 Mass Ave, Cambridge, MA 02139, USA.
 */
#define address_exception_op(a,b) printf("Address Check Violation\n");
#include <stdio.h>
#include <stdlib.h>
...
OP(jalr_op_0)
{
    ...
    if (pthread->getREG(picode, RS) & 0x3)
        ...
        fatal("target address (0x%x) of jalr instruction at addr 0x%x is past end of text.\n",
    		...
    		if ((signed int) pthread->getREG(picode, RS) < (Text_start)) {
    		fatal("target address (0x%x) of jalr instruction at addr ox%x is before the start of text.\n",
    				pthread->getREG(picode, RS), picode->addr);
    }
#endif
...
OP(lb_op_2)
{
  ...
  RAddr raddr = TMInterface::RedirectAddr(pthread->getPid(),true,pthread->getRAddr(),1);
  TMInterface::MarkLoad(picode,pthread->getPid(),pthread->real2virt(pthread->getRAddr()),1);
...
OP(ldc1_op_2)
{
  ...
  RAddr raddr = TMInterface::RedirectAddr(pthread->getPid(),true,pthread->getRAddr(),8);
  TMInterface::MarkLoad(picode,pthread->getPid(),pthread->real2virt(pthread->getRAddr()),8);

#ifdef ADDRESS_CHK
    if (pthread->getRAddr() & 7)
        address_exception_op(picode, pthread);
#endif
...
OP(lh_op_2)
{
  ...
  RAddr raddr = TMInterface::RedirectAddr(pthread->getPid(),true,pthread->getRAddr(),2);
  TMInterface::MarkLoad(picode,pthread->getPid(),pthread->real2virt(pthread->getRAddr()),2);

#ifdef ADDRESS_CHK
  if (pthread->getRAddr() & 1)
    address_exception_op(picode, pthread);
#endif
#ifdef LENDIAN
{
...
OP(ll_op_2)
{
  ...
  RAddr raddr = TMInterface::RedirectAddr(pthread->getPid(),true,pthread->getRAddr(),4);
  TMInterface::MarkLoad(picode,pthread->getPid(),pthread->real2virt(pthread->getRAddr()),4);

#ifdef ADDRESS_CHK
  if (pthread->getRAddr() & 3)
    address_exception_op(picode, pthread);
#endif
  /* read value from memory */
...
OP(lwl_op_2)
{
	...
	size_t aSize = (pthread->getRAddr() | 4) - pthread->getRAddr();
  RAddr raddr = TMInterface::RedirectAddr(pthread->getPid(),true,pthread->getRAddr(),aSize);
  TMInterface::MarkLoad(picode,pthread->getPid(),pthread->real2virt(pthread->getRAddr()),aSize);
...
OP(lwr_op_2)
{
uintptr_t augAddr = pthread->getRAddr() - (pthread->getRAddr() % 4);
	size_t aSize = 1 + (pthread->getRAddr() - augAddr);
  RAddr raddr = TMInterface::RedirectAddr(pthread->getPid(),true,augAddr,aSize);
  TMInterface::MarkLoad(picode,pthread->getPid(),pthread->real2virt(augAddr),aSize);

  /* read value from memory */
#ifdef LENDIAN
  pthread->setREG(picode, RT, mips_lwrLE(pthread->getREG(picode, RT), (char *)raddr + (aSize - 1)));
#else
  pthread->setREG(picode, RT, mips_lwrBE(pthread->getREG(picode, RT), (char *)raddr + (aSize - 1)));
#endif
...
OP(sb_op_2)
{
  ...
  RAddr raddr = TMInterface::RedirectAddr(pthread->getPid(),false,pthread->getRAddr(),1);
  TMInterface::MarkStore(picode,pthread->getPid(),pthread->real2virt(pthread->getRAddr()),1);
...
OP(sc_op_2)
{
  RAddr raddr = TMInterface::RedirectAddr(pthread->getPid(),false,pthread->getRAddr(),4);
  TMInterface::MarkStore(picode,pthread->getPid(),pthread->real2virt(pthread->getRAddr()),4);
...
{
    /* this is not an sc instruction */
  RAddr raddr = TMInterface::RedirectAddr(pthread->getPid(),false,pthread->getRAddr(),8);
  TMInterface::MarkStore(picode,pthread->getPid(),pthread->real2virt(pthread->getRAddr()),8);
...
#ifdef LENDIAN
{
  I(sizeof(double)==sizeof(long long));
  long long v1;
...
{
    /* this is not an sc instruction */
  RAddr raddr = TMInterface::RedirectAddr(pthread->getPid(),false,pthread->getRAddr(),2);
  TMInterface::MarkStore(picode,pthread->getPid(),pthread->real2virt(pthread->getRAddr()),2);
...
OP(sw_op_2)
{
  ...
  if (pthread->getRAddr() & 3)
    ...
    /* write value to memory */
#ifdef LENDIAN
  unsigned int v1 = pthread->getREG(picode, RT);
  unsigned int vTest = v1;
  v1 = SWAP_WORD(v1);
...
OP(swl_op_2)
{
    /* this is not an sc instruction */
  size_t aSize = (pthread->getRAddr() | 4) - pthread->getRAddr();
  RAddr raddr = TMInterface::RedirectAddr(pthread->getPid(),false,pthread->getRAddr(),aSize);
  TMInterface::MarkStore(picode,pthread->getPid(),pthread->real2virt(pthread->getRAddr()),aSize);
...
OP(swr_op_2)
{
 ...
 uintptr_t augAddr = pthread->getRAddr() - (pthread->getRAddr() % 4);
	size_t aSize = 1 + (pthread->getRAddr() - augAddr);
        RAddr raddr;
raddr = TMInterface::RedirectAddr(pthread->getPid(),false,augAddr,aSize);
  TMInterface::MarkStore(picode,pthread->getPid(),pthread->real2virt(augAddr),aSize);
  ...
  if (augAddr & 3)
    ...
    /* write value to memory */
#ifdef LENDIAN
  mips_swrLE(pthread->getREG(picode, RT), (char *)raddr + (aSize - 1));
#else
  mips_swrBE(pthread->getREG(picode, RT), (char *)raddr + (aSize - 1));
#endif
...
PFPI tm_simOp_op[] = { tm_simOp_op_0, tm_simOp_op_1 };

/* normal version */
OP(tm_transOp_op_0)
{
	I(picode->immed == 1 || picode->immed == 2 || picode->immed == 3);
	TMInterface::MarkTransOp(picode,pthread->getPid(),picode->immed,
		pthread->getREG(picode,1),
		pthread->getREG(picode,2),
		pthread->getREG(picode,3),
		pthread->getREG(picode,4),
		pthread->getREG(picode,5));
    return picode->next;
}
...
OP(tm_transOp_op_1)
{
	std::cout << "trans op in delay slot" << std::endl;
	I(0);
	TMInterface::MarkTransOp(picode,pthread->getPid(),picode->immed,
		pthread->getREG(picode,1),
		pthread->getREG(picode,2),
...
PFPI tm_transOp_op[] = { tm_transOp_op_0, tm_transOp_op_1 };

/* normal version */
OP(tm_specOp_op_0)
{
	TMInterface::MarkSpecOp(picode,pthread->getPid(),picode->immed,
		pthread->getREG(picode,1),
		pthread->getREG(picode,2),
		pthread->getREG(picode,3),
		pthread->getREG(picode,4),
...
PFPI tm_specOp_op[] = { tm_specOp_op_0, tm_specOp_op_1 };
