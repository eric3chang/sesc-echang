...
/*
	...
	*/
	class MSICache : public BaseMemDevice
	{
	private:
		...
		typedef	PooledFunctionGenerator<StoredClassFunction2<MSICache, const BaseMsg*, int, &MSICache::RetryMsg> > CBRetryMsg;
		CBRetryMsg cbRetryMsg;

		void PrepareFreshBlock(int set, int index, AddrTag tag);
		typedef	PooledFunctionGenerator<StoredClassFunction3<MSICache, int, int, AddrTag, &MSICache::PrepareFreshBlock> > CBPrepareFreshBlock;
		CBPrepareFreshBlock cbPrepareFreshBlock;

		void EvictBlock(AddrTag tag);
		typedef	PooledFunctionGenerator<StoredClassFunction1<MSICache, AddrTag, &MSICache::EvictBlock> > CBEvictBlock;
		CBEvictBlock cbEvictBlock;

		void PerformRead(const ReadMsg* m);
		typedef PooledFunctionGenerator<StoredClassFunction1<MSICache, const ReadMsg*, &MSICache::PerformRead> > CBPerformRead;
		CBPerformRead cbPerformRead;

		void PerformRemoteRead(const ReadMsg* m);
		typedef PooledFunctionGenerator<StoredClassFunction1<MSICache, const ReadMsg*, &MSICache::PerformRemoteRead> > CBPerformRemoteRead;
		CBPerformRemoteRead cbPerformRemoteRead;

		void PerformWrite(const WriteMsg* m);
		typedef PooledFunctionGenerator<StoredClassFunction1<MSICache, const WriteMsg*, &MSICache::PerformWrite> > CBPerformWrite;
