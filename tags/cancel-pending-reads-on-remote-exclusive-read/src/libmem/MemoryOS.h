...
//  std::vector<MemRequest *> optionalReqs;
...
public:
  ...
  virtual bool insertTransAccess(bool isRead, bool isRequired, VAddr addr, size_t size, CallbackBase* cb)
  {
	  if(isRequired || !busy)
	  {
		  solveRequest(CBMemRequest::create(0,memorySystem->getDataSource(),(isRead)?MemRead:MemWrite,addr,cb));
		  return true;
	  }
	  return false;
  }
