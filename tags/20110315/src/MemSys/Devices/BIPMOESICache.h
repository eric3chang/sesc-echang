...
/*
	...
	*/
	class BIPMOESICache : public BaseMemDevice
	{
	private:
		...
		typedef	PooledFunctionGenerator<StoredClassFunction2<BIPMOESICache, const BaseMsg*, int, &BIPMOESICache::RetryMsg> > CBRetryMsg;
		CBRetryMsg cbRetryMsg;

		void PrepareFreshBlock(int set, int index, AddrTag tag);
		typedef	PooledFunctionGenerator<StoredClassFunction3<BIPMOESICache, int, int, AddrTag, &BIPMOESICache::PrepareFreshBlock> > CBPrepareFreshBlock;
		CBPrepareFreshBlock cbPrepareFreshBlock;

		void EvictBlock(AddrTag tag);
		typedef	PooledFunctionGenerator<StoredClassFunction1<BIPMOESICache, AddrTag, &BIPMOESICache::EvictBlock> > CBEvictBlock;
		CBEvictBlock cbEvictBlock;

		void PerformRead(const ReadMsg* m);
		typedef PooledFunctionGenerator<StoredClassFunction1<BIPMOESICache, const ReadMsg*, &BIPMOESICache::PerformRead> > CBPerformRead;
		CBPerformRead cbPerformRead;

		void PerformRemoteRead(const ReadMsg* m);
		typedef PooledFunctionGenerator<StoredClassFunction1<BIPMOESICache, const ReadMsg*, &BIPMOESICache::PerformRemoteRead> > CBPerformRemoteRead;
		CBPerformRemoteRead cbPerformRemoteRead;

		void PerformWrite(const WriteMsg* m);
		typedef PooledFunctionGenerator<StoredClassFunction1<BIPMOESICache, const WriteMsg*, &BIPMOESICache::PerformWrite> > CBPerformWrite;
