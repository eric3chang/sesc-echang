...
public:
  ...
  virtual void doReadBank(MemRequest *mreq);
  virtual void doRead(MemRequest *mreq);
  virtual void doReadQueued(MemRequest *mreq);
  virtual void doWriteBank(MemRequest *mreq);
  virtual void doWrite(MemRequest *mreq);
  virtual void doWriteQueued(MemRequest *mreq);
  void activateOverflow(MemRequest *mreq);

  virtual void readMissHandler(MemRequest *mreq);
  virtual void writeMissHandler(MemRequest *mreq);

  void wbuffAdd(PAddr addr);
  void wbuffRemove(PAddr addr);
  bool isInWBuff(PAddr addr);

  virtual Line *allocateLine(PAddr addr, CallbackBase *cb);
  virtual void doAllocateLine(PAddr addr, PAddr rpl_addr, CallbackBase *cb);
  virtual void doAllocateLineRetry(PAddr addr, CallbackBase *cb);
...
public:
  ...
  size_t getLineSize() const { return (size_t)cacheBanks[0]->getLineSize(); }
  PAddr calcLineAddr(PAddr addr) const { return (addr / getLineSize()) * getLineSize(); }
  ...
  virtual bool isMOESI(){return false;}//used by moesi directory protocol & cache stack
};
