...
// 		 sizeof(MemT),(long long)(readMem<MemT>(context,addr)),addr,inst->addr,inst->name);
#if (defined EMUL_VALGRIND)
	if(addr==0)
	  if(inst==0)
	    fail("Never\n");
...
// 		 sizeof(MemT),(long long)(getReg<MemT,S2Typ>(context,inst->regSrc2)),addr,inst->addr,inst->name);
