...
static void parse_args(int32_t argc, char **argv)
{
  ...
  Stack_size = STACK_SIZE;
  if(SescConf->checkInt("","HeapMemorySize"))
  {
	  Heap_size = SescConf->getInt("","HeapMemorySize");
  }
  else
  {
	  Heap_size = HEAP_SIZE;
...
icode_ptr newcopy_icode(icode_ptr picode){
   ...
   */
  if (Nicode == 0) {
    Icode_free = (icode_ptr) malloc(1024 * sizeof(icode));
    if (Icode_free == NULL)
...
read_text()
{
  ...
  icode_ptr picode, prev_picode, dslot, *pitext;
...
//	picode->target = picode + 1;
  //    } else {
    ...
    if(opnum == tm_simOp_opn)
	{
		picode->func = simOpHandler(picode);
	}
    ...
    if (prev_was_branch) {
      ...
      dslot->next = NULL;
		...
		if(opnum == tm_simOp_opn)
		{
			dslot->func = simOpHandler(picode);
		}
      /* Either not tracing any memory references or
    ...
    for (j = 0; j < 4; j++) {
	  ...
	  if (regfield[j] == 0) {
                if (iflags & MOD0_IS_NOP) {
					...
					else if(opnum == ori_opn)
					{
						opnum = tm_transOp_opn;
					}
					else if(opnum == xori_opn)
					{
						opnum = tm_specOp_opn;
					}
					else
...
void DumpProgram()
{
	std::ofstream pout("program.txt",std::ios::out);
	int limit = Text_size + EXTRA_ICODES;
	for(int i = 0; i < limit; i++)
	{
		icode* p = Itext[i];
		pout << "Addr:" << p->addr << " " <<  p->dis_instr() << std::endl;
	}
	pout.close();
}
