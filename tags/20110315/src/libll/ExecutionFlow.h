...
public:
  ...
  void SimOpBeginRabbit()
  {
	  simOpRabbitModeCounter++;
	  if(simOpRabbitModeCounter == 1)
	  {
		  goRabbitMode();
	  }
  }
  ...
  {
	  simOpRabbitModeCounter--;
	  if(simOpRabbitModeCounter == 0)
	  {
		  goingRabbit = false;
	  }
	  else if(simOpRabbitModeCounter < 0)
	  {
